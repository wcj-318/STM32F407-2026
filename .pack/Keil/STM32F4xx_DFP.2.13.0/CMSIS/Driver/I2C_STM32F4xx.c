  ...
  if (sr1 & I2C_SR1_SB) {
    ...
    if (tr->ctrl & XFER_CTRL_DMA_DONE) {
      ...
      if (tr->status.mode == I2C_MODE_MASTER) {
        ...
        if (tx->num != 0) {
          ...
          if ((uint32_t)tx->cnt == tx->num) {
            ...
            tr->ctrl |= XFER_CTRL_XDONE | XFER_CTRL_WAIT_BTF;
          }
        }
        else {
      ...
      if (i2c->info->status.mode == I2C_MODE_MASTER) {
        ...
        if (tr->ctrl & XFER_CTRL_WAIT_BTF) {
          if (sr1 & I2C_SR1_BTF) {
            if ((rx->num == 2U) || ((uint32_t)rx->cnt == (rx->num - 2U))) {
              /* Two bytes remaining */
              i2c->reg->CR2 &= ~I2C_CR2_ITBUFEN;
...
static void I2C_DMA_TxEvent (uint32_t event, I2C_RESOURCES *i2c) {
  ...
  i2c->info->tx.cnt = (int32_t)(i2c->info->tx.num - __HAL_DMA_GET_COUNTER(i2c->dma_tx->h));

  if (i2c->info->status.mode) {
    /* Master transmitter: Wait for BTF in I2C EV IRQ handler */
    i2c->info->ctrl |= XFER_CTRL_DMA_DONE;
  }
}
#endif
...
static void I2C_DMA_RxEvent (uint32_t event, I2C_RESOURCES *i2c) {
  ...
  i2c->info->rx.cnt = (int32_t)(i2c->info->rx.num - __HAL_DMA_GET_COUNTER(i2c->dma_rx->h));
